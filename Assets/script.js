...
//list that will hold the answer list
...
var questionObj = { //question object that holds all the parts of questions
    questions: [ //questions can just be added to by adding on a string to end of array
        ...
        'To make use of an Array, do they have to be the same data type?',
     ...
     // There are multiple answers for one question          
    answers: [ 
        ['<script.js>', '<js>', '<script>', '<javascript>'],
        ['typeof', 'datatype', 'data', 'typeData'],
        ['maybe', 'no', 'yes', 'no idea'], 
        ['Boolean', 'true and false', 'arithmic properties', 'parenthesis'],
        ['Footer', 'Header', 'Head', 'Body'] 
    ] 
}
...
//Setup buttons for start of game
function setUpGame() {
    timeLeft = globalTimerPreset; 
    timerTag.textContent = globalTimerPreset; 

    //this would be the last visible item after viewing highscore of a previous game
    document.querySelector('#display-highscore-div').style.display = 'none'; 
    //fills back content that gets reused for quiz questions 
    titleTag.textContent = 'Javascript Code Quiz'; 

    //show the quiz title because after 1 round it will be hidden
...
//gets triggered if the start button is pressed 
function startGame() {
    // gameEnded = false; 
    questionIndexNumber = 0; 

    //when game starts the high score button disappears
    viewHighscoresBtn.style.display = 'none' 
    //the start button is hidden when game begins
    startBtn.style.display = 'none'; 
    document.querySelector('#instructions').style.display = 'none'; 
    //shows timer at the top since game started
...
//Shows the question of the current index and its answers
function showQuestions(currentQuestionIndex) {
    //shows the next question in h1 font
    titleTag.textContent = questionObj.questions[currentQuestionIndex]; 
    //shows answers for said question
...
function createAnswerElements(currentQuestionIndex) {
    //clears out all current answers for new epic round of answers to be shown
...
function endGame() { 
    gameEnded = true; 
    //leftover time when game ends
    score = timeLeft; 

    //hide necessary elements
    //hide timer on end screen since game is over
    timerPTag.style.display = 'none'; 
    //this hides the title 
    titleTag.style.display = 'none'; 
    //new question doesn't get generated
...
function checkAnswer(event) {
    if (event.target != answerButtonList){ //if this is just the <ul> dont do anything else we just want the <li> (answers) themselves not parent
...
function clearHighscores() {
    //empties out the highscore list 
    document.querySelector('ol').innerHTML = ''; 
    //erases all local storage
...
function init() {
    ...
    //list that contains the answer <li> tags which are used as buttons
